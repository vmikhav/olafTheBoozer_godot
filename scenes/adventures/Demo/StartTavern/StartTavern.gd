...
func _ready():
	...
	if !StoryProgress.tavern_flashback:
		next_scene = ["res://scenes/game/Playground/Playground.tscn", {levels = ["Demo/TavernTutorial"]}]
	elif !StoryProgress.janitor_resque_requested:
		janitor_quest(3)
		hero_start_position = Vector2i(-1, -2)
		move_hero_to_position(hero_start_position)
		next_scene = ["res://scenes/game/Playground/Playground.tscn", {levels = ["Demo/Kitchen"]}]
	elif !StoryProgress.janitor_resque_stage2:
		kitchen_door_key()
		hero_start_position = Vector2i(-4, -6)
		move_hero_to_position(hero_start_position)
		hero.set_orientation('right')
		next_scene = ["res://scenes/game/Playground/Playground.tscn", {levels = ["Demo/Library"]}]
	elif !StoryProgress.janitor_resqued:
		kitchen_door_lockpick()
		hero_start_position = Vector2i(-4, -6)
		move_hero_to_position(hero_start_position)
		hero.set_orientation('right')
		next_scene = ["res://scenes/game/Playground/Playground.tscn", {levels = ["Demo/TavernKitchen"]}]
