...
func _ready():
	...
	if !StoryProgress.sawmill_demon_captured:
		next_scene = ["res://scenes/game/Playground/Playground.tscn", {levels = ["Demo/SawmillDemon"]}]
		intro()
	elif !StoryProgress.sawmill_warehouse_fixed:
		next_scene = ["res://scenes/game/Playground/Playground.tscn", {levels = ["Demo/SawmillWorkshop"]}]
		demon_captured()
	else:
